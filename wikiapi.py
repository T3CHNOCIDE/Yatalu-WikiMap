...
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
	...
	This module was heavily influenced by Hairr's MediaWiki API using the 
	Python requests module instead of the urllib module. This module has
	some methods cut down and some new methods added compared to Hairr's 
...
def allpages(limit=None, namespace=None):
	"""
	Gets all pages on wiki.
	Can be specified to within a given namespace and/or limit.
	"""
	...
	if limit:
		list_data['aplimit'] = limit
	
	if namespace:
		list_data['apnamespace'] = namespace
	
	response = session.post(wikia_site, data=list_data)
	content = json.loads(response.content)
	...
	list_members = content['query']['allpages']
	list_pages = []
	
	while True:
	
		for page in list_members:
			list_pages.append(page["title"])	
		
		if "query-continue" in content.keys():
	
			list_data['apfrom'] = content["query-continue"]["allpages"]["apfrom"]
...
def site_info(site):
	"""
	Pulls basic site data for current wiki.
	"""
	...
	lang_site = "http://" + site + ".wikia.com/api.php"
	
	response = session.post(lang_site, data=lang_page_data)
	content = json.loads(response.content)
